import { Button, cn, Divider, Drawer, DrawerBody, DrawerContent, DrawerProps, useDisclosure } from '@heroui/react'
  ...
  return (
    <>
      ...
      >
        <span
          className={cn(
            'absolute top-1/7 left-0 w-full rounded-xl border border-black duration-300',
            isOpen && 'translate-y-1.25 rotate-45',
          )}
        ></span>
        <span
          className={cn(
            'transfrom absolute top-1/2 left-0 w-full -translate-y-1/2 rounded-xl border border-black duration-300',
            isOpen && '-left-10',
          )}
        ></span>
        <span
          className={cn(
            'absolute bottom-1/7 left-0 w-full rounded-xl border border-black duration-300',
            isOpen && '-translate-y-1.25 -rotate-45',
          )}
        ></span>
      </div>
